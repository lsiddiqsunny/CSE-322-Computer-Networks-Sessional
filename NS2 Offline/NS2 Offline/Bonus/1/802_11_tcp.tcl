...
set ftp_pckt_per_sec  [lindex $argv 2]; # [expr 100*$factor]
set Tx_Range     [lindex $argv 3];# [expr 5*$factor]
set ftp_interval [expr 1.0/$ftp_pckt_per_sec] ;# ?????? 1 for 1 packets per second and 0.1 for 10 packets per second
...
for {set i 0} {$i < $num_flow } {incr i} {
     $ns_ at [expr $start_time+$i*$flow_start_gap] "$ftp_($i) start"
}
